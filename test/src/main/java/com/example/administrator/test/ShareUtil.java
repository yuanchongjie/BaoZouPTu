...
public class ShareUtil {
    /**
     * @param appInfos   所有符合要求应用的Item信息
     * @param preferApps 优先旋转的应用Activity的title，越前面，优先级越高
     * @return
     */
    public static List<ListDrawableItem> getSortedAppData(List<ListDrawableItem> appInfos, List<CharSequence> preferApps) {
        ...
        for (int p = size; p >= 0; p--) {
            ...
            for (int i = 0; i < appInfos.size(); i++) {
                if (appInfos.get(i).getTitle().equals(cs)) {
                    ListDrawableItem item = appInfos.get(i);
                    appInfos.remove(i);
                    appInfos.add(0, item);
                    break;
                }
